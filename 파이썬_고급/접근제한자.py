# 접근 제한자란? 클래스 내부에 선언된 변수에 대한 접근을 제한 하는 것
# 내부 변수를 외부에서 접근 하고자 할 때 지정된 게터와 세터 메소드를 통해서만 접근 하도록 하는 것
# private(__) : 같은 클래스 내에서만 접근 가능
# protected(_) : 같은 클래스 내이거나 상속관계가 존재하면 접근 가능
# public() : 아무데서나 접근 가능
...
class TV:
    ...
    def __init__(self, name, isOn, channel, volume):
        self.__name = name      # private : 클래스 내부에서만 접근 가능
        self.__isOn = isOn
